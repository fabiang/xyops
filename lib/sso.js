			...
			self.storage.get(path, function(err, user) {
				...
				var finish = function() {
					...
					if (header_map.groups) {
						var group_role_separator = sso.group_role_separator || ",";
						var raw_groups = external_user.groups ? external_user.groups.split(
							new RegExp(`${group_role_separator.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\s*`)
						) : [];
